...
//       if (!info[key].toString().trim()) {
//         toast.error("Missing fields");
//         return; // ✅ prevent submit if missing
//       }
//     }
//     const infoUpdated = { ...info, role: info.role || null };
//     if (type === "add") {
...
import { baseUrl } from "../../../constants/global-variable.js";
import { queryClient } from "../../../utils/queryClient.js";
import { apiFetch } from "../../../utils/api.js"; // ✅ use apiFetch
...
const InputEmployee = ({ children, type = "add", data }) => {
  ...
  const addEmployeeMutation = useMutation({
    mutationFn: async (info) => {
      // ✅ apiFetch auto adds token + refresh
  ...
  const updateMutation = useMutation({
    mutationFn: async (info) => {
      return await apiFetch(`${baseUrl}/${info.id}`, {
        method: "PUT",
        body: JSON.stringify(info),
        headers: { "Content-Type": "application/json" },
      });
